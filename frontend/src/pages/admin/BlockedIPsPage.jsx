...
const BlockedIPsPage = ({ data, onBlockIP, onUnblockIP, onBlockFingerprint, onUnblockFingerprint, backendOnline }) => {
  const [activeTab, setActiveTab] = useState('ips') // 'ips' | 'fingerprints'
  ...
  const fetchBlocklists = async () => {
    ...
    try {
      const resIP = await fetch(`${BACKEND}/api/blocklist`)
      const dIP = await resIP.json()
      if (Array.isArray(dIP)) setBlockedList(dIP)

      const resFP = await fetch(`${BACKEND}/api/blocklist/fingerprints`)
  ...
  const handleUnblockFP = async (fp) => {
    if (onUnblockFingerprint) {
      ...
      try {
        await fetch(`${BACKEND}/api/blocklist/fingerprint/${encodeURIComponent(fp)}`, { method: 'DELETE' })
      } catch {}
    }
    await fetchBlocklists()
  }
    ...
    <div style={{ padding:'24px' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
        <div>
          <h2 style={{ color:'#E6EDF3', margin:0, fontSize:'20px', fontWeight:700 }}>
            🛡️ Threat Blocklist Management
          </h2>
          <p style={{ color:'#8B949E', fontSize:'13px', margin:'4px 0 0' }}>
            Multi-signal blocklist tracking IPs and unique browser fingerprints across proxy networks
          </p>
        </div>
      ...
      <div style={{ display:'flex', gap:'8px', borderBottom:'1px solid #30363D', marginBottom:'20px' }}>
        <button onClick={() => setActiveTab('ips')}
          style={{ padding:'10px 20px', background:'transparent', border:'none', borderBottom: activeTab === 'ips' ? '3px solid #00FF88' : '3px solid transparent', color: activeTab === 'ips' ? '#00FF88' : '#8B949E', fontWeight: activeTab === 'ips' ? 700 : 500, fontSize:'14px', cursor:'pointer' }}>
          🚫 Blocked IPs ({allBlockedIPs.length})
        </button>
        <button onClick={() => setActiveTab('fingerprints')}
          style={{ padding:'10px 20px', background:'transparent', border:'none', borderBottom: activeTab === 'fingerprints' ? '3px solid #00FF88' : '3px solid transparent', color: activeTab === 'fingerprints' ? '#00FF88' : '#8B949E', fontWeight: activeTab === 'fingerprints' ? 700 : 500, fontSize:'14px', cursor:'pointer' }}>
          ...
          {filteredIPs.length === 0 ? (
            ...
            <div style={{ ...cardStyle, padding:0, overflow:'hidden' }}>
              <div style={{ display:'grid', gridTemplateColumns:'2fr 1.5fr 1.5fr 2fr 1.5fr 1fr 1fr auto', padding:'10px 16px', background:'#0D1117', borderBottom:'1px solid #30363D' }}>
                {['IP Address / Fingerprint','Country','City','Reason','Blocked At','Attacks','Attempts','Actions'].map(h => (
                  <div key={h} style={{ color:'#8B949E', fontSize:'11px', fontWeight:600, letterSpacing:'0.05em' }}>{h}</div>
                ))}
              </div>
              {filteredIPs.map((b, i) => (
                <div key={b.ip || i}
                  style={{ display:'grid', gridTemplateColumns:'2fr 1.5fr 1.5fr 2fr 1.5fr 1fr 1fr auto', padding:'12px 16px', borderBottom: i < filteredIPs.length-1 ? '1px solid #21262D' : 'none', alignItems:'center', transition:'background 0.15s' }}
