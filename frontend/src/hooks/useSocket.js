...
export const useSocket = ({ onAttack, onAlert, onSessionUpdate, onHoney, onAutoResponse, onSessionRemoved, onSessionBlocked, onBlockedAttempt, onIPBlocked, onIPUnblocked, onFingerprintBlocked, onVPNDetected }) => {
  const socketRef = useRef(null)
  const [connected, setConnected] = useState(false)
  const [latency, setLatency] = useState(null)

  const handlersRef = useRef({
    onAttack, onAlert, onSessionUpdate, onHoney, onAutoResponse,
    onSessionRemoved, onSessionBlocked, onBlockedAttempt,
    onIPBlocked, onIPUnblocked, onFingerprintBlocked, onVPNDetected
  })
  ...
  useEffect(() => {
    ...
    socket.on('new_attack',          data => { console.log('[Socket] new_attack:', data.attack?.type); handlersRef.current.onAttack?.(data) })
    socket.on('new_alert',           data => { console.log('[Socket] new_alert:', data.title); handlersRef.current.onAlert?.(data) })
    socket.on('session_updated',     data => { console.log('[Socket] session_updated:', data.username, 'riskScore:', data.riskScore); handlersRef.current.onSessionUpdate?.(data) })
    ...
    socket.on('vpn_detected',         data => { console.log('[Socket] vpn_detected:', data); handlersRef.current.onVPNDetected?.(data) })
